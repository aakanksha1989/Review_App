  ...
  render() {
    ...
    for (const key in SORT_OPTIONS) {
      let displayOption = (
        <div key={key}>
          <input
            type="radio"
            onChange={this.selectSortFilter}
            id={key}
            name="sort_options"
            value={key}></input>
          <label className="dropdown_show_options" htmlFor={key}>
            {SORT_OPTIONS[key]}
          </label>
          <br />
        </div>
    ...
    return (
      <div className="dropdown">
        <button onClick={this.props.onSortClick} className="dropdown_button">
          SORT BY
        </button>
        <div
          id="myDropdown"
          className={this.props.open ? 'dropdown_show' : 'dropdown_hide'}>
          {options}
        </div>
      </div>
    );
